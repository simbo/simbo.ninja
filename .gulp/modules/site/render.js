...
const renderer = require('app/modules/renderer');
...
function renderSite(options) {
  ...
  function transform(file, enc, done) {
    if ((/\.pug$/i).test(file.history[0])) {
      ...
      file.contents = new Buffer(renderer.pug.render(String(file.contents), merge({}, options.pug, file.data)));
    } else if ((/\.(md|markdown)$/i).test(file.history[0])) {
      file.contents = new Buffer(renderer.marked(String(file.contents)));
    }
  ...
  function applyLayout(file, done) {
    getLayout(file.data.layout).done((layout) => {
      if (layout) {
        ...
        file.contents = new Buffer(renderer.pug.render(layout.contents, merge({}, options.pug, file.data)));
      }
      done(null, file);
    }, done);
  }
